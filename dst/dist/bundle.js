...
function generateFragShader(customShapes) {
    const customSDFs = [0, 1, 2]
        .map((i) => {
        const shape = customShapes[i];
        const code = shape ? shape.sdfCode : "return 1e9;";
        return `
float sdCustom${i}(vec4 p, vec4 size) {
    ${code}
}`;
    })
        .join("\n");
...
uniform float uGlobalRot[6];

#define MAX_OBJECTS 8
uniform int   uObjectCount;
uniform int   uObjType[MAX_OBJECTS];
uniform vec4  uObjPos[MAX_OBJECTS];
uniform vec4  uObjSize[MAX_OBJECTS];
uniform float uObjRot[MAX_OBJECTS * 6];
uniform int   uObjCsg[MAX_OBJECTS];
uniform float uObjSmooth[MAX_OBJECTS];
uniform vec3  uObjColor[MAX_OBJECTS];

vec4 rotXY(vec4 p, float a){ float c=cos(a),s=sin(a); return vec4(c*p.x-s*p.y, s*p.x+c*p.y, p.z, p.w); }
vec4 rotXZ(vec4 p, float a){ float c=cos(a),s=sin(a); return vec4(c*p.x-s*p.z, p.y, s*p.x+c*p.z, p.w); }
vec4 rotXW(vec4 p, float a){ float c=cos(a),s=sin(a); return vec4(c*p.x-s*p.w, p.y, p.z, s*p.x+c*p.w); }
vec4 rotYZ(vec4 p, float a){ float c=cos(a),s=sin(a); return vec4(p.x, c*p.y-s*p.z, s*p.y+c*p.z, p.w); }
...
vec4 applyRot(vec4 p, float r[6]){
  p = rotXY(p, r[0]); p = rotXZ(p, r[1]); p = rotXW(p, r[2]);
  p = rotYZ(p, r[3]); p = rotYW(p, r[4]); p = rotZW(p, r[5]);
  return p;
}

vec4 applyGlobalRot(vec4 p){ return applyRot(p, uGlobalRot); }
...
float sdSpheritorus(vec4 p, float R, float r){ return length(vec2(length(p.xyz) - R, p.w)) - r; }
float sdTiger(vec4 p, float R, float r){ return length(vec2(length(p.xy) - R, length(p.zw) - R)) - r; }
float sdDuocylinder(vec4 p, vec2 r){ return max(length(p.xy) - r.x, length(p.zw) - r.y); }
float sdFiveCell(vec4 p, float s){
  float d = dot(p, vec4(1,1,1,-1)) - s;
  d = max(d, dot(p, vec4(1,-1,-1,-1)) - s);
  d = max(d, dot(p, vec4(-1,1,-1,-1)) - s);
  d = max(d, dot(p, vec4(-1,-1,1,-1)) - s);
  d = max(d, dot(p, vec4(-1,-1,-1,1)) - s);
  return d * 0.5;
}
float sdSixteenCell(vec4 p, float s){ return (abs(p.x) + abs(p.y) + abs(p.z) + abs(p.w) - s) * 0.577; }
...
float sdObject(vec4 p, int type, vec4 size){
  if (type == 0)  return sdSphere4(p, size.x);
  if (type == 1)  return sdBox4(p, size);
  if (type == 2)  return sdSpheritorus(p, size.x, size.y);
  if (type == 3)  return sdTiger(p, size.x, size.y);
  if (type == 4)  return sdDuocylinder(p, size.xy);
  if (type == 5)  return sdFiveCell(p, size.x);
  if (type == 6)  return sdSixteenCell(p, size.x);
  if (type == 7)  return sdBox4(p, vec4(size.x));
  if (type == 8)  return sdCustom0(p, size);
  if (type == 9)  return sdCustom1(p, size);
  if (type == 10) return sdCustom2(p, size);
  return 1e9;
}

float map(vec4 p){
  vec4 pg = applyGlobalRot(p);
  float result = 1e9;
  for (int i = 0; i < MAX_OBJECTS; i++){
    if (i >= uObjectCount) break;
    vec4 lp = pg - uObjPos[i];
    float r[6];
    int base = i * 6;
    r[0] = uObjRot[base+0]; r[1] = uObjRot[base+1]; r[2] = uObjRot[base+2];
    r[3] = uObjRot[base+3]; r[4] = uObjRot[base+4]; r[5] = uObjRot[base+5];
    lp = applyRot(lp, r);
    float d = sdObject(lp, uObjType[i], uObjSize[i]);
    int op = uObjCsg[i];
    float k = uObjSmooth[i];
    if (i == 0) result = d;
    else {
      if (op == 0) result = min(result, d);
      else if (op == 1) result = max(result, -d);
      else if (op == 2) result = max(result, d);
      else if (op == 3) result = smin(result, d, k);
    }
  }
  return result;
}

vec3 getColorAtHit(vec4 p){
  vec4 pg = applyGlobalRot(p);
  float minDist = 1e9; int closestIdx = 0;
  for (int i = 0; i < MAX_OBJECTS; i++){
    if (i >= uObjectCount) break;
    vec4 lp = pg - uObjPos[i];
    float r[6]; int base = i * 6;
    r[0] = uObjRot[base+0]; r[1] = uObjRot[base+1]; r[2] = uObjRot[base+2];
    r[3] = uObjRot[base+3]; r[4] = uObjRot[base+4]; r[5] = uObjRot[base+5];
    lp = applyRot(lp, r);
    float d = sdObject(lp, uObjType[i], uObjSize[i]);
  ...
  return uObjColor[closestIdx];
}

vec4 calcNormal(vec4 p){
  const vec2 e = vec2(0.002, 0.0);
  return normalize(vec4(map(p+e.xyxx)-map(p-e.xyxx), map(p+e.yxyx)-map(p-e.yxyx), map(p+e.xxyx)-map(p-e.xxyx), map(p+e.xxxy)-map(p-e.xxxy)));
}

float raymarch(vec4 ro, vec4 rd, out vec4 hitP){
  float t = 0.0;
  for (int i = 0; i < 256; i++){
    if (i >= uMaxSteps) break;
    vec4 p = ro + rd * t;
    float d = map(p);
    if (d < uSurfDist) { hitP = p; return t; }
    if (t > uMaxDist) break;
    t += d * 0.7;
  }
  return -1.0;
...
void main(){
  ...
  vec4 ro = vec4(uCameraPos, uCameraW);
  vec4 rd = normalize(vec4(uv * uFov, 1.0, 0.0));
  rd = applyGlobalRot(rd);
  vec4 hitP;
  float t = raymarch(ro, rd, hitP);
  vec3 col;
  if (t < 0.0){ col = vec3(0.02, 0.03, 0.05); }
  else {
    vec4 n = calcNormal(hitP);
    vec3 lightDir = normalize(uLightDir);
    float diff = max(dot(n.xyz, lightDir), 0.0);
    diff = mix(diff, diff + 0.3 * max(n.w, 0.0), 0.5);
    vec3 baseColor = getColorAtHit(hitP);
    col = baseColor * (uAmbient + diff) * uLightColor;
    float ao = 1.0 - float(raymarch(hitP + n*0.02, n, hitP) < 0.0) * 0.15;
    col *= ao;
...
class Renderer {
    constructor(canvas) {
        ...
        this.gl = gl;
        this.vertShader = compile(gl, gl.VERTEX_SHADER, vertSrc);
        this.rebuildShader([]);
    ...
    rebuildShader(customShapes) {
        const gl = this.gl;
        if (this.program)
            gl.deleteProgram(this.program);
        const fragSrc = generateFragShader(customShapes);
        const fs = compile(gl, gl.FRAGMENT_SHADER, fragSrc);
        this.program = link(gl, this.vertShader, fs);
        gl.deleteShader(fs);
    ...
    updateCustomShapes(shapes) {
        this.rebuildShader(shapes);
    }
    resize() {
    ...
    render(scene, time) {
        ...
        gl.uniform3f(this.loc["uCameraPos"], scene.camera.position.x, scene.camera.position.y, scene.camera.position.z);
        gl.uniform1f(this.loc["uCameraW"], scene.camera.w);
        gl.uniform3f(this.loc["uLightDir"], scene.light.direction.x, scene.light.direction.y, scene.light.direction.z);
        gl.uniform3f(this.loc["uLightColor"], scene.light.color.r, scene.light.color.g, scene.light.color.b);
        gl.uniform1f(this.loc["uAmbient"], scene.light.ambient);
        gl.uniform1f(this.loc["uFov"], scene.render.fov);
        gl.uniform1i(this.loc["uMaxSteps"], scene.render.maxSteps);
        gl.uniform1f(this.loc["uMaxDist"], scene.render.maxDist);
        gl.uniform1f(this.loc["uSurfDist"], scene.render.surfDist);
        gl.uniform1fv(this.loc["uGlobalRot[0]"], new Float32Array([
            scene.globalRot.xy,
            scene.globalRot.xz,
            scene.globalRot.xw,
            scene.globalRot.yz,
        ...
        const objs = scene.objects.slice(0, 8);
        gl.uniform1i(this.loc["uObjectCount"], objs.length);
        const types = new Int32Array(8);
        const pos = new Float32Array(8 * 4);
        const size = new Float32Array(8 * 4);
        const rot = new Float32Array(8 * 6);
        const csg = new Int32Array(8);
        const smooth = new Float32Array(8);
        const colors = new Float32Array(8 * 3);
        objs.forEach((o, i) => {
            types[i] = o.type;
            pos[i * 4 + 0] = o.position[0];
            pos[i * 4 + 1] = o.position[1];
            pos[i * 4 + 2] = o.position[2];
            pos[i * 4 + 3] = o.position[3];
            size[i * 4 + 0] = o.size[0];
            size[i * 4 + 1] = o.size[1];
            size[i * 4 + 2] = o.size[2];
            size[i * 4 + 3] = o.size[3];
            rot[i * 6 + 0] = o.rotation[0];
            rot[i * 6 + 1] = o.rotation[1];
            rot[i * 6 + 2] = o.rotation[2];
            rot[i * 6 + 3] = o.rotation[3];
            rot[i * 6 + 4] = o.rotation[4];
            rot[i * 6 + 5] = o.rotation[5];
            csg[i] = o.csg;
            smooth[i] = o.smoothness;
            colors[i * 3 + 0] = o.color.r;
            colors[i * 3 + 1] = o.color.g;
            colors[i * 3 + 2] = o.color.b;
        });
        gl.uniform1iv(this.loc["uObjType[0]"], types);
        gl.uniform4fv(this.loc["uObjPos[0]"], pos);
        gl.uniform4fv(this.loc["uObjSize[0]"], size);
        gl.uniform1fv(this.loc["uObjRot[0]"], rot);
        gl.uniform1iv(this.loc["uObjCsg[0]"], csg);
        gl.uniform1fv(this.loc["uObjSmooth[0]"], smooth);
        gl.uniform3fv(this.loc["uObjColor[0]"], colors);
...
var ShapeType;
(function (ShapeType) {
    ShapeType[ShapeType["Sphere4"] = 0] = "Sphere4";
    ShapeType[ShapeType["Tesseract"] = 1] = "Tesseract";
    ShapeType[ShapeType["Spheritorus"] = 2] = "Spheritorus";
    ShapeType[ShapeType["Tiger"] = 3] = "Tiger";
    ShapeType[ShapeType["Duocylinder"] = 4] = "Duocylinder";
    ShapeType[ShapeType["FiveCell"] = 5] = "FiveCell";
    ShapeType[ShapeType["SixteenCell"] = 6] = "SixteenCell";
    ShapeType[ShapeType["EightCell"] = 7] = "EightCell";
    ShapeType[ShapeType["Custom0"] = 8] = "Custom0";
    ShapeType[ShapeType["Custom1"] = 9] = "Custom1";
    ShapeType[ShapeType["Custom2"] = 10] = "Custom2";
})(ShapeType || (ShapeType = {}));
const SHAPE_NAMES = {
    [ShapeType.Sphere4]: "Sphere4D",
    [ShapeType.Tesseract]: "Tesseract",
    [ShapeType.Spheritorus]: "Spheritorus",
    [ShapeType.Tiger]: "Tiger",
    [ShapeType.Duocylinder]: "Duocylinder",
    [ShapeType.FiveCell]: "FiveCell",
    [ShapeType.SixteenCell]: "SixteenCell",
    [ShapeType.EightCell]: "EightCell",
    [ShapeType.Custom0]: "Custom0",
    [ShapeType.Custom1]: "Custom1",
    [ShapeType.Custom2]: "Custom2",
};
var CsgOp;
(function (CsgOp) {
    CsgOp[CsgOp["Union"] = 0] = "Union";
    CsgOp[CsgOp["Subtract"] = 1] = "Subtract";
    CsgOp[CsgOp["Intersect"] = 2] = "Intersect";
    CsgOp[CsgOp["SmoothUnion"] = 3] = "SmoothUnion";
})(CsgOp || (CsgOp = {}));
let nextId = 1;
const createObject = (type = ShapeType.Tesseract, color = { r: 0.85, g: 0.55, b: 0.35 }) => ({
    id: nextId++,
    type,
    position: [0, 0, 0, 0],
...
const createScene = () => ({
    render: { maxSteps: 128, maxDist: 20, surfDist: 0.002, fov: 1.2 },
    camera: { position: { x: 0, y: 0, z: -4 }, w: -2 },
    light: {
        direction: { x: 0.5, y: 0.8, z: 0.4 },
        color: { r: 1, g: 0.95, b: 0.85 },
        ambient: 0.25,
    },
    globalRot: { xy: 0, xz: 0, xw: 0.4, yz: 0, yw: 0.3, zw: 0 },
    objects: [createObject(ShapeType.Tesseract)],
});
...
class UI {
    constructor(scene, onChange, toggleAutoRotate, getAutoRotate) {
        this.scene = scene;
        this.onChange = onChange;
        this.toggleAutoRotate = toggleAutoRotate;
        this.getAutoRotate = getAutoRotate;
        this.expandedStates = new Map();
        this.customShapes = [];
        this.customShapeNames = ["Custom0", "Custom1", "Custom2"];
        this.onCustomShapesChange = () => { };
        this.objPane = new Pane({ title: "Objects", container: document.body });
        this.objPane.element.style.position = "fixed";
        this.objPane.element.style.left = "8px";
        this.objPane.element.style.top = "40px";
        this.objPane.element.style.maxHeight = "calc(100vh - 50px)";
        this.objPane.element.style.overflowY = "auto";
        const customFolder = this.objPane.addFolder({ title: "Custom Shapes" });
        customFolder
            .addButton({ title: "Load Custom Shape 1 (JSON)" })
            .on("click", () => this.loadCustomShape(0));
        customFolder
            .addButton({ title: "Load Custom Shape 2 (JSON)" })
            .on("click", () => this.loadCustomShape(1));
        customFolder
            .addButton({ title: "Load Custom Shape 3 (JSON)" })
            .on("click", () => this.loadCustomShape(2));
        this.objPane.addButton({ title: "+ Add Object" }).on("click", () => {
            scene.objects.push(createObject(ShapeType.Sphere4));
            this.rebuildObjects();
            onChange();
        });
        this.objFolder = this.objPane.addFolder({ title: "Object Params" });
        this.rebuildObjects();
        this.paramsPane = new Pane({
            title: "Settings",
            container: document.body,
        });
        this.paramsPane.element.style.position = "fixed";
        this.paramsPane.element.style.right = "8px";
        this.paramsPane.element.style.top = "40px";
        this.paramsPane.element.style.maxHeight = "calc(100vh - 50px)";
        this.paramsPane.element.style.overflowY = "auto";
        this.paramsPane
            .addButton({ title: "Toggle Auto-Rotate" })
            .on("click", () => {
            toggleAutoRotate();
            this.refresh();
        });
        const render = this.paramsPane.addFolder({ title: "Render" });
        render
            .addBinding(scene.render, "maxSteps", {
            min: 16,
            max: 512,
            step: 1,
        })
            .on("change", onChange);
        render
            .addBinding(scene.render, "maxDist", {
            min: 1,
            max: 100,
            step: 0.25,
        })
            .on("change", onChange);
        render
            .addBinding(scene.render, "surfDist", {
            min: 0.00005,
            max: 0.05,
            step: 0.00005,
        })
            .on("change", onChange);
        render
            .addBinding(scene.render, "fov", { min: 0.1, max: 4, step: 0.05 })
            .on("change", onChange);
        const cam = this.paramsPane.addFolder({ title: "Camera" });
        cam.addBinding(scene.camera.position, "x", {
            min: -50,
            max: 50,
            step: 0.05,
        }).on("change", onChange);
        cam.addBinding(scene.camera.position, "y", {
            min: -50,
            max: 50,
            step: 0.05,
        }).on("change", onChange);
        cam.addBinding(scene.camera.position, "z", {
            min: -50,
            max: 50,
            step: 0.05,
        }).on("change", onChange);
        cam.addBinding(scene.camera, "w", { min: -20, max: 20, step: 0.05 }).on("change", onChange);
        const light = this.paramsPane.addFolder({ title: "Light" });
        light
            .addBinding(scene.light.direction, "x", {
            min: -1,
            max: 1,
            step: 0.01,
        })
            .on("change", onChange);
        light
            .addBinding(scene.light.direction, "y", {
            min: -1,
            max: 1,
            step: 0.01,
        })
            .on("change", onChange);
        light
            .addBinding(scene.light.direction, "z", {
            min: -1,
            max: 1,
            step: 0.01,
        })
            .on("change", onChange);
        light
            .addBinding(scene.light.color, "r", { min: 0, max: 3, step: 0.01 })
            .on("change", onChange);
        light
            .addBinding(scene.light.color, "g", { min: 0, max: 3, step: 0.01 })
            .on("change", onChange);
        light
            .addBinding(scene.light.color, "b", { min: 0, max: 3, step: 0.01 })
            .on("change", onChange);
        light
            .addBinding(scene.light, "ambient", { min: 0, max: 1, step: 0.01 })
            .on("change", onChange);
        const rot = this.paramsPane.addFolder({ title: "4D Global Rotation" });
        rot.addBinding(scene.globalRot, "xy", {
            min: -Math.PI * 2,
            max: Math.PI * 2,
            step: 0.005,
        }).on("change", onChange);
        rot.addBinding(scene.globalRot, "xz", {
            min: -Math.PI * 2,
            max: Math.PI * 2,
            step: 0.005,
        }).on("change", onChange);
        rot.addBinding(scene.globalRot, "xw", {
            min: -Math.PI * 2,
            max: Math.PI * 2,
            step: 0.005,
        }).on("change", onChange);
        rot.addBinding(scene.globalRot, "yz", {
            min: -Math.PI * 2,
            max: Math.PI * 2,
            step: 0.005,
        }).on("change", onChange);
        rot.addBinding(scene.globalRot, "yw", {
            min: -Math.PI * 2,
            max: Math.PI * 2,
            step: 0.005,
        }).on("change", onChange);
        rot.addBinding(scene.globalRot, "zw", {
            min: -Math.PI * 2,
            max: Math.PI * 2,
            step: 0.005,
        }).on("change", onChange);
    }
    setOnCustomShapesChange(callback) {
        this.onCustomShapesChange = callback;
    }
    loadCustomShape(index) {
        ...
        input.onchange = async (e) => {
            ...
            try {
                ...
                if (!data.name || !data.sdfCode) {
                    alert("Invalid JSON format. Required fields: name, sdfCode");
                    return;
                }
                const customShape = {
                    name: data.name,
                    sdfCode: data.sdfCode,
                };
                this.customShapes[index] = customShape;
                this.customShapeNames[index] = data.name;
                const customType = ShapeType.Custom0 + index;
                SHAPE_NAMES[customType] = data.name;
                this.rebuildObjects();
                this.onCustomShapesChange([
                    ...this.customShapes.filter((s) => s),
                ]);
                alert(`Custom shape "${data.name}" loaded successfully!`);
            }
            catch (err) {
                alert("Error loading JSON file: " + err);
            }
        };
        input.click();
    }
    refresh() {
        this.objPane.refresh?.();
        this.paramsPane.refresh?.();
    }
    rebuildObjects() {
        if (this.objFolder) {
            this.objFolder.dispose();
        }
        this.objFolder = this.objPane.addFolder({ title: "Object Params" });
        this.scene.objects.forEach((obj, idx) => {
            const wasExpanded = this.expandedStates.get(obj.id) ?? false;
            const f = this.objFolder.addFolder({
                title: `#${idx} ${SHAPE_NAMES[obj.type]}`,
                expanded: wasExpanded,
            });
            const folderElement = f.element;
            if (folderElement) {
                const observer = new MutationObserver(() => {
                    const isCollapsed = folderElement.classList.contains("tp-fldv-cpl");
                    this.expandedStates.set(obj.id, !isCollapsed);
                });
            ...
            for (const k of shapeKeys)
                shapeOpts[SHAPE_NAMES[k]] = k;
            f.addBinding(obj, "type", {
                options: shapeOpts,
                label: "Shape",
            }).on("change", (e) => {
                f.title = `#${idx} ${SHAPE_NAMES[e.value]}`;
                this.onChange();
            });
            const colorFolder = f.addFolder({ title: "Color" });
            colorFolder
                .addBinding(obj.color, "r", { min: 0, max: 1, step: 0.01 })
                .on("change", this.onChange);
            colorFolder
                .addBinding(obj.color, "g", { min: 0, max: 1, step: 0.01 })
                .on("change", this.onChange);
            colorFolder
                .addBinding(obj.color, "b", { min: 0, max: 1, step: 0.01 })
                .on("change", this.onChange);
            f.addBinding(obj.position, "0", {
                min: -10,
                max: 10,
                step: 0.005,
                label: "pos.x",
            }).on("change", this.onChange);
            f.addBinding(obj.position, "1", {
                min: -10,
                max: 10,
                step: 0.005,
                label: "pos.y",
            }).on("change", this.onChange);
            f.addBinding(obj.position, "2", {
                min: -10,
                max: 10,
                step: 0.005,
                label: "pos.z",
            }).on("change", this.onChange);
            f.addBinding(obj.position, "3", {
                min: -10,
                max: 10,
                step: 0.005,
                label: "pos.w",
            }).on("change", this.onChange);
            f.addBinding(obj.size, "0", {
                min: 0.01,
                max: 5,
                step: 0.005,
                label: "size.x",
            }).on("change", this.onChange);
            f.addBinding(obj.size, "1", {
                min: 0.01,
                max: 5,
                step: 0.005,
                label: "size.y",
            }).on("change", this.onChange);
            f.addBinding(obj.size, "2", {
                min: 0.01,
                max: 5,
                step: 0.005,
                label: "size.z",
            }).on("change", this.onChange);
            f.addBinding(obj.size, "3", {
                min: 0.01,
                max: 5,
                step: 0.005,
                label: "size.w",
            }).on("change", this.onChange);
            f.addBinding(obj.rotation, "0", {
                min: -Math.PI * 2,
                max: Math.PI * 2,
                step: 0.005,
                label: "rot.XY",
            }).on("change", this.onChange);
            f.addBinding(obj.rotation, "1", {
                min: -Math.PI * 2,
                max: Math.PI * 2,
                step: 0.005,
                label: "rot.XZ",
            }).on("change", this.onChange);
            f.addBinding(obj.rotation, "2", {
                min: -Math.PI * 2,
                max: Math.PI * 2,
                step: 0.005,
                label: "rot.XW",
            }).on("change", this.onChange);
            f.addBinding(obj.rotation, "3", {
                min: -Math.PI * 2,
                max: Math.PI * 2,
                step: 0.005,
                label: "rot.YZ",
            }).on("change", this.onChange);
            f.addBinding(obj.rotation, "4", {
                min: -Math.PI * 2,
                max: Math.PI * 2,
                step: 0.005,
                label: "rot.YW",
            }).on("change", this.onChange);
            f.addBinding(obj.rotation, "5", {
                min: -Math.PI * 2,
                max: Math.PI * 2,
                step: 0.005,
                label: "rot.ZW",
            }).on("change", this.onChange);
            f.addBinding(obj, "csg", {
                options: {
                    Union: CsgOp.Union,
                    Subtract: CsgOp.Subtract,
                    Intersect: CsgOp.Intersect,
                    SmoothUnion: CsgOp.SmoothUnion,
                },
                label: "CSG",
            }).on("change", this.onChange);
            f.addBinding(obj, "smoothness", {
                min: 0.01,
                max: 2.0,
                step: 0.01,
                label: "Smoothness",
            }).on("change", this.onChange);
            f.addButton({ title: "Delete Object" }).on("click", () => {
...
const ui = new UI(scene, markDirty, toggleAutoRotate, getAutoRotate);
ui.setOnCustomShapesChange((shapes) => {
    renderer.updateCustomShapes(shapes);
});
const keys = {};
...
canvas.addEventListener("wheel", (e) => {
    ...
    scene.camera.position.x += sinY * cosX * speed;
    scene.camera.position.y -= sinX * speed;
    scene.camera.position.z -= cosY * cosX * speed;
    ui.refresh();
}, { passive: false });
function updateCameraFromInput(dt) {
    ...
    const cosX = Math.cos(camRotX);
    if (keys["KeyW"]) {
        scene.camera.position.x += sinY * cosX * moveSpeed;
        scene.camera.position.z -= cosY * cosX * moveSpeed;
        ui.refresh();
    }
    if (keys["KeyS"]) {
        scene.camera.position.x -= sinY * cosX * moveSpeed;
    ...
    if (keys["KeyA"]) {
        scene.camera.position.x -= cosY * moveSpeed;
        scene.camera.position.z -= sinY * moveSpeed;
        ui.refresh();
    }
    if (keys["KeyD"]) {
        scene.camera.position.x += cosY * moveSpeed;
...
function applyCameraRotation() {
    if (!autoRotate) {
        scene.globalRot.xw = camRotY;
        scene.globalRot.yw = camRotX;
    }
}
...
function loop() {
    ...
    if (autoRotate) {
        scene.globalRot.xw = 0.4 + t * 0.15;
        scene.globalRot.yw = 0.3 + t * 0.12;
    }
    else {
        applyCameraRotation();
    }
...
if (info) {
    info.textContent =
        "WASD: move | Space/Ctrl: up/down | Q/E: W-axis | Mouse: rotate | Wheel: zoom";
}
